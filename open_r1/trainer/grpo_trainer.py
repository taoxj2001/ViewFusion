...
import sys
sys.path.insert(0, "/data/user/xtao467/data/code/SIMS-VSI/Image2Video")
...
class Qwen3VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            if "Qwen3-VL" in model_id or "Qwen2.5-VL" in model_id or "Aria" in model_id or True:
    ...
    def _get_train_sampler(self, train_dataset: Dataset | None = None) -> torch.utils.data.Sampler | None:
        print("rank/world_size:", torch.distributed.get_rank(), torch.distributed.get_world_size())
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # prompts_text = maybe_apply_chat_template(input_copy[0], self.processing_class)['prompt']
        # print(prompts_text)

        # prompts_text = [maybe_apply_chat_template(example, self.processing_class) for example in input_copy]
        ...
        prompt_inputs.pop("token_type_ids", None) # 新增
        ...
        if self.temporal and video_inputs:
            ...
            shuffled_prompt_inputs.pop("token_type_ids", None) # 新增
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            prompt_completion_ids = unwrapped_model.generate(**prompt_inputs,temperature=1.0, pad_token_id=self.pad_token_id, eos_token_id=[151645, 151645, 151643],generation_config=self.generation_config)
            ...
            if self.temporal:
                if video_inputs:
                    shuffled_prompt_completion_ids = unwrapped_model.generate(**shuffled_prompt_inputs,temperature=1.0, pad_token_id=self.pad_token_id,eos_token_id=[151645, 151645, 151643],  generation_config=self.shuffled_generation_config)
                    ...
                    shuffled_prompt_completion_ids = unwrapped_model.generate(**prompt_inputs, temperature=1.0, pad_token_id=self.pad_token_id, eos_token_id=[151645, 151645, 151643],generation_config=self.dummy_generation_config)
